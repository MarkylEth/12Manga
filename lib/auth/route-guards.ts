...
/* ====================== Config ====================== */
/** Разрешён ли DEV-override по x-user-id (header/cookie). */
function isDevHeaderAllowed(): boolean {
  return String(process.env.AUTH_ALLOW_DEV_HEADER || '').trim() === '1'
}

// Логирование статуса только в development
if (process.env.NODE_ENV === 'development') {
  // eslint-disable-next-line no-console
  console.log(`[AUTH] DEV headers ${isDevHeaderAllowed() ? 'ENABLED' : 'DISABLED'}`)
}
...
/** Пробуем достать UUID из заголовка/куки (только если isDevHeaderAllowed() === true) */
async function getViewerIdFromHeaderOrCookie(
  req?: Request | NextRequest
): Promise<string | null> {
  if (!isDevHeaderAllowed()) return null
 ...
 * 0) если разрешён DEV-override — сначала x-user-id (header/cookie)
 * 1) если не сработал — основная система (`coreGetAuthUser`)
...
export async function getAuthUser(
  ...
  // 0) DEV-override (приоритетно, только если разрешено)
  ...
  try {
    const t = await query<{ team_id: number }>(
      `select team_id
         from public.translator_team_members
        where user_id = $1::uuid
          and coalesce(role,'') in ('lead','leader')
        limit 1`,
      [userId]
    )
...
export async function getAuthUserWithPermissions(
  ...
  return {
    ...
    isAdmin:
      ((profile?.role ?? u.role ?? 'user') as string).toLowerCase() === 'admin',
  }
}
...
export async function requireLoggedIn(
  ...
  if (!user)
    return { ok: false, status: 401, reason: 'unauthorized', user: null }
...
export async function requireRole(
  ...
  if (!allowed.includes(role))
    return { ok: false, status: 403, reason: 'forbidden', user: u }
  return { ok: true, status: 200, reason: null, user: u }
}

/** Разрешаем ТОЛЬKO по ролям admin/moderator (без x-api-key) */
...
export async function getAuthUserLegacy(
  req: Request | NextRequest
