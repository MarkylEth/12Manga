...
export default function TeamClientPage({ slug }: { slug: string }): JSX.Element {
  ...
  const loadTeam = async (isFirst: boolean, withTitles: boolean = true) => {
    ...
    try {
      // 1) Team
      const rTeam = await fetch(`/api/teams/${encodeURIComponent(slug)}`, { cache: 'no-store', signal })
      if (!rTeam.ok) {
        if (signal.aborted) return
        setTeam(null); setMembers([]); setTitles([]); return
      }
      ...
      if (withTitles && t?.id && !signal.aborted) {
        try {
          const r2 = await fetch(`/api/teams/${encodeURIComponent(slug)}/titles`, { cache: 'no-store', signal })
          if (signal.aborted) return
          if (r2.ok) {
        ...
        } catch { if (!signal.aborted) setTitles([]) }
      }

      // 3) Stats
      if (t?.id && !signal.aborted) {
        try {
          const rStats = await fetch(`/api/teams/${encodeURIComponent(slug)}/stats`, { cache: 'no-store', signal })
      ...
      if (t?.id && !signal.aborted) {
        void (async () => {
          ...
          try {
            ...
            if (!rMem.ok) throw new Error(`HTTP ${rMem.status}`)
            const jm = await rMem.json()
            const items = Array.isArray(jm?.items) ? jm.items : []
            if (!signal.aborted) setMembers(items as MemberWithProfile[])
          } catch (e) {
